#### Role name: 
    configure_gshadow_minus_permissions
...
#### Description:
    This Ansible role ensures that the `/etc/gshadow-` file — which stores backup group account information including hashed passwords and other critical security data — has correct ownership and restrictive permissions to prevent unauthorized access. This role addresses security rule **35768** (Wazuh).
...
#### Requirements            
    ...
    - Linux distributions using `/etc/gshadow-` (e.g., Debian, Ubuntu, RHEL, CentOS, SUSE)

#### Variables

| Variable            | Default         | Description                                             | file              |
|---------------------|-----------------|---------------------------------------------------------|-------------------|
| `expected_group`    | `shadow`        | Required group ownership of the file (fallback: `root`) | defaults/main.yml |
| `expected_mode`     | `0640`          | Owner permission code (r/w/)                            | defaults/main.yml |
| `expected_owner`    | `root`          | Required owner (user) of the file                       | defaults/main.yml |
| `gshadow_file_path` | `/etc/gshadow-` | Path to the gshadow backup file                         | defaults/main.yml |
