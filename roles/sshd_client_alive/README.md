...
#### Requirements
    ...
    - `sshd` service must be present and configurable
    - `sshd -T` command must be available for validation

#### Variables

| Variable                      | Default                  | Description                                                              | file              |
|-------------------------------|--------------------------|--------------------------------------------------------------------------|-------------------|
| `sshd_client_alive_interval`  | `15`                     | Timeout interval in seconds for SSH client alive messages                | defaults/main.yml |
| `sshd_client_alive_count_max` | `3`                      | Maximum number of unacknowledged client alive messages before disconnect | defaults/main.yml |
| `sshd_config_file`            | `/etc/ssh/sshd_config`   | Path to the main sshd configuration file                                 | defaults/main.yml |
| `sshd_include_dir`            | `/etc/ssh/sshd_config.d` | Directory for additional sshd configuration files                        | defaults/main.yml |
| `sshd_service_name`           | `sshd`                   | Name of the SSH daemon service (used for restart)                        | vars/main.yml     |
| `sshd_package_name`           | `openssh-server`         | Package name for OpenSSH server                                          | vars/main.yml     |
