...
#### Description:
    This Ansible role ensures that the /etc/cron.monthly directory has secure ownership and permissions to prevent unauthorized access. The directory contains system cron jobs that run monthly and must be protected from tampering by non-privileged users.
...
#### Requirements            
    ...
    - Linux-based systems with `/etc/cron.monthly` directory present (typically systems with `cron` or `cronie` installed)

#### Variables

| Variable | Default | Description | file |
|---------|---------|-------------|------|
| `cron_monthly_path` | `/etc/cron.monthly` | Path to the cron monthly directory | defaults/main.yml |
| `cron_monthly_owner` | `root` | Owner of the directory | defaults/main.yml |
| `cron_monthly_group` | `root` | Group of the directory | defaults/main.yml |
| `cron_monthly_mode` | `0700` | Permission mode for the directory (octal string) | defaults/main.yml |
